...
def create_and_activate_posts(author, number_of_posts)
  number_of_posts.downto(1).each do |index|
    Post.create!(author:, title: "Post #{index} by #{author.name}", text: 'My post text')
  end
end

def create_and_activate_user(name)
  user = User.create!(name:, email: "#{name}@example.com", password: '123456', photo: "#{name}.jpg")
  user.confirm
  user.save
  user
end
...
describe 'Post Index', type: :feature do
  ...
  it 'shows the first comment on a post' do
    expect(page).to have_content "Comment 1 made by #{@user.name} on post #{@user.posts.first.id}"
  end
